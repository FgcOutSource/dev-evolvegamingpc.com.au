 ...
 (() => {
   document.addEventListener("DOMContentLoaded", function() {
    console.log("DOM is fully loaded!");
     setTimeout(() => {
        document.querySelectorAll('.mm-submenu').forEach(el => {
    el.insertAdjacentHTML('afterend', '<div class="overlay"></div>');
});
}, 2000);
...
`;
  ...
  var InputNumber = class extends HTMLInputElement {
    ...
    set quantity(quantity) {
      ...
      let parentQuantityForm = this.closest('.product-form__quantity');
    
    if (parentQuantityForm) {
        let quantityInputs =  document.querySelectorAll('.product-form__quantity .quantity-selector__input');
